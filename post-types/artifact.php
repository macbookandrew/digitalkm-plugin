...
function artifact_init() {
	register_post_type( 'artifact', array(
		'labels'                => array(
...
function artifact_updated_messages( $messages ) {
	...
	$messages['artifact'] = array(
		...
		5  => isset( $_GET['revision'] ) ? sprintf( __( 'Historical Artifact restored to revision from %s', 'digitalkm' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
		/* translators: %s: post permalink */
		...
		9  => sprintf( __( 'Historical Artifact scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview Historical Artifact</a>', 'digitalkm' ),
